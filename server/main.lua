...
-- Vérifie si le joueur peut voir les caméras
...
-- Vérifie si le joueur peut gérer les caméras
...
RegisterCommand(Config.PlacementCommand, function()
    ...
    -- Vérifier le grade pour la gestion (CanManage)
    if Config.Framework == 'esx' then
        local playerJob = PlayerData.job.name
        local playerGrade = PlayerData.job.grade
        local allowedGrades = Config.ManageCamerasGrades[playerJob]
        ...
        if not canManage then
            TriggerEvent('chat:addMessage', {
                color = { 255, 0, 0 },
                args = { "Law System", "You don't have the required grade to place cameras." }
            })
            return
        end
    end
