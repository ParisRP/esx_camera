...
function UpdatePermissions()
    ...
    -- Vérifier si peut voir les caméras
    local canView = false
    if Config.ViewCamerasGrades[jobName] then
        for _, allowedGrade in pairs(Config.ViewCamerasGrades[jobName]) do
    ...
    -- Vérifier si peut gérer les caméras
    CanManage = false
    if Config.ManageCamerasGrades[jobName] then
        for _, allowedGrade in pairs(Config.ManageCamerasGrades[jobName]) do
            if jobGrade == allowedGrade then
                CanManage = true
                break
            end
...
function GetReadableZoneName(coords)
...
function StartCameraPlacement()
    ...
    while not HasModelLoaded(model) do
        ...
        if timeout > 400 then
            ...
            ESX.ShowNotification("~r~Échec du chargement du modèle de caméra.")
            return
        end
    end
    ...
    Citizen.CreateThread(function()
        while IsPlacingCamera do
            ...
            if IsDisabledControlPressed(0, 33) then -- S
                local heading = math.rad(camHeading + 180)
                SetEntityCoords(TempCamera,
                    camCoords.x + math.sin(heading) * 0.05,
                    camCoords.y + math.cos(heading) * 0.05,
                    camCoords.z, false, false, false, false)
            ...
            if IsDisabledControlPressed(0, 172) then -- Flèche haut
                local camRot = GetEntityRotation(TempCamera, 2)
                SetEntityRotation(TempCamera, camRot.x + 1.0, camRot.y, camRot.z, 2, true)
            end
            if IsDisabledControlPressed(0, 173) then -- Flèche bas
                local camRot = GetEntityRotation(TempCamera, 2)
                SetEntityRotation(TempCamera, camRot.x - 1.0, camRot.y, camRot.z, 2, true)
            end
            if IsDisabledControlPressed(0, 174) then -- Flèche gauche
                SetEntityHeading(TempCamera, camHeading + 2.0)
            end
            if IsDisabledControlPressed(0, 175) then -- Flèche droite
                SetEntityHeading(TempCamera, camHeading - 2.0)
            end

            -- Confirmer
            if IsDisabledControlJustPressed(0, 201) or IsDisabledControlJustPressed(0, 191) then -- ENTER
                SendNUIMessage({
                    action = "showPlacementHUD",
                    show = false
                })
...
function ConfirmCameraPlacement()
    if not TempCamera or not DoesEntityExist(TempCamera) then
        IsPlacingCamera = false
        return
    end
    ...
    -- Pas de screenshot, on procède directement
    ...
    ESX.ShowNotification("~g~Caméra placée avec succès!")
end
...
-- Spawn des objets caméra dans le monde
...
-- Marquage de la station de monitoring
Citizen.CreateThread(function()
    while true do
        Wait(0)
        ...
        if distance < Config.MonitoringStation.drawDistance then
            ...
            if distance < Config.MonitoringStation.interactionDistance then
                DrawText3D(stationCoords.x, stationCoords.y, stationCoords.z + 0.5,
                    "~b~[E]~w~ Accéder au système de caméras")
...
function ViewCamera(cameraId)
    ...
    if BrokenCameras[cameraId] then
        SendNUIMessage({
            ...
            message = "Cette caméra est actuellement hors ligne en raison de dommages"
        })
        return
    end
    ...
    Citizen.CreateThread(function()
        ...
        while ViewingCamera do
            ...
            -- Zoom
            if IsDisabledControlPressed(0, Config.ZoomInKey) then -- Numpad +
                CurrentZoom = math.max(20.0, CurrentZoom - 0.5)
                SetCamFov(cam, CurrentZoom)
            end
            if IsDisabledControlPressed(0, Config.ZoomOutKey) then -- Numpad -
                CurrentZoom = math.min(80.0, CurrentZoom + 0.5)
                SetCamFov(cam, CurrentZoom)
            end

            -- Vision nocturne
            if IsDisabledControlJustPressed(0, Config.NightVisionKey) then
            ...
            -- Vision thermique
            if IsDisabledControlJustPressed(0, Config.ThermalVisionKey) then
                ThermalVisionActive = not ThermalVisionActive
                SetSeethrough(ThermalVisionActive)
            ...
            if elapsedTime > 500 then
                if IsDisabledControlJustPressed(0, 194) or IsDisabledControlJustPressed(0, 202) then -- BACKSPACE ou ESC
                    ExitCameraView()
                    break
                end
            end
...
-- Commandes
RegisterCommand(Config.PlacementCommand, function()
    if not IsPolice() then
        ESX.ShowNotification("~r~Vous n'avez pas la permission de placer des caméras.")
        return
    end

    if not CanManage then
        ESX.ShowNotification("~r~Vous n'avez pas le grade requis pour placer des caméras.")
        return
    end

    TriggerEvent('lspd_cameras:requestCameraName')
end, false)
...
-- Événements réseau
RegisterNetEvent('lspd_cameras:startPlacement')
AddEventHandler('lspd_cameras:startPlacement', function()
    StartCameraPlacement()
end)

RegisterNetEvent('lspd_cameras:updateCameras')
AddEventHandler('lspd_cameras:updateCameras', function(cameras, alerts, canManage)
...
RegisterNetEvent('lspd_cameras:alertUpdate')
AddEventHandler('lspd_cameras:alertUpdate', function(cameraId, alertData)
...
RegisterNetEvent('lspd_cameras:removeCamera')
AddEventHandler('lspd_cameras:removeCamera', function(cameraId)
...
RegisterNetEvent('lspd_cameras:destroyAllCameras')
AddEventHandler('lspd_cameras:destroyAllCameras', function()
    local cameraModels = {
...
RegisterNetEvent('lspd_cameras:requestCameraName')
AddEventHandler('lspd_cameras:requestCameraName', function()
    SetNuiFocus(true, true)
...
RegisterNetEvent('lspd_cameras:cameraStatusUpdate')
AddEventHandler('lspd_cameras:cameraStatusUpdate', function(cameraId, brokenData, alertData)
...
RegisterNetEvent('lspd_cameras:cameraRepaired')
AddEventHandler('lspd_cameras:cameraRepaired', function(cameraId)
...
RegisterNetEvent('lspd_cameras:syncBrokenCameras')
AddEventHandler('lspd_cameras:syncBrokenCameras', function(brokenCameras)
    for id, data in pairs(brokenCameras) do
        BrokenCameras[id] = data.broken
    end
end)

RegisterNetEvent('lspd_cameras:showCameraScreenshot')
AddEventHandler('lspd_cameras:showCameraScreenshot', function(cameraId, screenshot)
    SendNUIMessage({
...
Citizen.CreateThread(function()
    ...
    print("[LSPD Cameras] Client ESX initialisé avec succès")
end)